...
export async function handleKpis(ctx: ApiContext): Promise<ApiResponse> {
  ...
  try {
    ...
    // Do not leak internal details; log would go to App Insights in production
    return {
      status: 500,
      headers: baseHeaders(correlationId),
      body: errorBody("INTERNAL_ERROR", "Failed to load KPIs", correlationId),
    };
  }
}
